...
##          Please fully read and test any scripts before running in your production environment!
    ...
    #>
#Requires -modules mmicrosoft.graph, importexcel, exchangeonlinemanagement
Param(
...
Try {
    ...
    clear
    write-host "Could not import modules, make sure you have the following modules available`nMicrosoft.Graph`nExchangeOnlineManagement`nimportexcel`nThese modules can be installed by running the following commands:`nInstall-Module Microsoft.Graph`n`nInstall-Module ExchangeOnlineManagement`n`nInstall-Module importexcel" -ForegroundColor red
    exit
}
...
if ($templatePresent -eq $false) {
    write-host "Template file not found, please ensure the template file is present in the c:\TenantAssessment directory" -ForegroundColor red
    pause
    exit
}
...
$users = Get-MgUser -Property id, userPrincipalName, mail, displayName, givenName, surname, licenseAssignmentStates, proxyAddresses, usageLocation, userType, accountEnabled
...
foreach ($teamgroup in $TeamGroups) {
    ...
    $TotalTeamSize = $privatechannelSize + $sharedchannelSize + $teamgroup.Datasize

    If ($TotalTeamSize -ne 0) {
    
        $TotalTeamSize = [Math]::Round(((($TotalTeamSize / 1024) / 1024) / 1024), 2)
        if ($TotalTeamSize -eq 0) {
            $TotalTeamSize = 0.01
    ...
    $teamgroup | Add-Member -MemberType NoteProperty -Name "TotalTeamSize(GB)" -Value $TotalTeamSize -Force
...
$SharePoint | Add-Member -MemberType NoteProperty -Name "Storage Used (GB)" -Value "" -force
foreach ($Site in $Sharepoint) {

    $Site.TeamID = ($TeamGroups | ? { $_.url -contains $site.'site url' }).id
    If ($site.'Storage Used (Byte)' -ne 0) {
    
        $Site.'Storage Used (GB)' = [Math]::Round(((($Site.'Storage Used (Byte)' / 1024) / 1024) / 1024), 2)
        if ($Site.'Storage Used (GB)' -eq 0) {
            $Site.'Storage Used (GB)' = 0.01
